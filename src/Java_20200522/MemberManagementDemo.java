...
public class MemberManagementDemo {
	...
	public void run(String inputData) {
		if (inputData.contentEquals("1")) {
			...
			print();
			String inputId = input("번호를 선택하세요>");
			run(inputId);
		}
	}
	...
	//1. 수정할 아이디를 입력받는다.
	//2. ArrayList에 있는 고객정보에서 수정할 아이디를 찾아서 수정할 이름을
	...
	//1. 검색할 아이디를 입력 받는다.
	//2. ArrayList 에 Member 객체 중에서 검색할 아이디를 찾아서 있으면 출력하고
	//   없으면검색된 결과가 없습니다." 메세지 출력
	//3. ArrayList 에 있는 모든 고객을 출력한다.
	//4. print() 메서드 호출
	//5. 번호를 선택하세요> 메세지를 출력한 후 키보드 입력을 받게 한다.
	public void search() {
